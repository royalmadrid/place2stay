...
const Rooms = ({ setSelectedLink, link }) => {
  const {state:{rooms, currentUser }, dispatch} = useValue();
  ...
  const columns = useMemo(()=>[
    {
      field: 'images', headerName: 'Photo', width: 70,
  ...
  return (
    ...
    >
      ...
      columns={columns}
      rows={
        isAdmin(currentUser) ? rooms : rooms.filter(room=>room.uid === currentUser.id)
      }
